...
//	public void callQuestionSelectionProc(@Param("quiz_id") Long quiz_id);
	...
	@Query(" from TrnQuizHeaderModel where employeeModel.emp_code=:emp_code and quiz_status='A' and  quiz_end_time>=systimestamp and quiz_start_time<=systimestamp and quiz_taken='Y'"
			+ " and  employeeModel.current_divn_cd=:div_code and quiz_id not in (select old_quiz_id from TrnQuizResetLogModel)  "
			+ " and employeeModel.curr_comp_code in (:allowed_comp_code)  order by quiz_start_time desc ")
	public List<TrnQuizHeaderModel> findDivQuizListForResetCompCode(@Param("emp_code") Long emp_code, String div_code,
